...
(() => {
    ...
    const setupHowToOrderModal = () => {
        ...
        const modalTitle = document.querySelector(
            'meta[name="how-to-order-title"]',
        )?.content;
        const modalDescription = document.querySelector(
            'meta[name="how-to-order-description"]',
        )?.content;
    ...
    const setupAccountMenus = () => {
        ...
        const closeAll = () => {
            menus.forEach((menu) => {
                ...
                menu.querySelector("[data-account-toggle]")?.setAttribute(
                    "aria-expanded",
                    "false",
                );
            });
        ...
        document
            ...
            .forEach((button) => {
                ...
                button.addEventListener("click", () => {
                    ...
                    button.setAttribute(
                        "aria-label",
                        isVisible ? "Lihat password" : "Sembunyikan password",
                    );
                    button.setAttribute(
                        "title",
                        isVisible ? "Lihat password" : "Sembunyikan password",
    ...
    const setupProfileTabs = () => {
        document.querySelectorAll("[data-profile-tabs]").forEach((tabs) => {
            ...
            const requestedTab =
                window.location.hash === "#ubah-password"
                    ? "ubah-password"
    ...
    const setupParticipantTypeFields = () => {
        document
            .querySelectorAll("[data-participant-type]")
            .forEach((select) => {
                ...
                const updateFields = () => {
                    categories.forEach((category) => {
                        const isActive =
                            category.dataset.registrationCategory ===
                            select.value;
                        ...
                        category
                            .querySelectorAll("[data-category-field]")
                            .forEach((input) => {
    ...
    const setupRegistrationValidation = () => {
        document
            .querySelectorAll("[data-registration-confirm]")
            .forEach((form) => {
                ...
                form.querySelectorAll("[data-gmail-field]").forEach((field) => {
                    bindValidation(
                        field,
                        "Gunakan alamat Gmail yang valid, misalnya nama@gmail.com.",
                    );
                });

                form.querySelectorAll("[data-indonesian-phone]").forEach(
                    (field) => {
                        bindValidation(
                            field,
                            "Gunakan nomor Indonesia, misalnya 081234567890 atau 6281234567890.",
                        );
                    },
                );
                ...
                form.querySelectorAll("[data-text-only]").forEach((field) => {
                    ...
                    const validateText = () => {
                        ...
                        field.setCustomValidity(
                            containsNumber
                                ? "Kolom ini tidak boleh berisi angka."
                                : "",
                        );
                    };
    ...
    const setupCustomRegistrationFields = () => {
        const openCategoryPanel = (categoryTab) => {
            ...
            eventCard
                ?.querySelectorAll("[data-schema-category-tab]")
                .forEach((tab) => {
                    tab.setAttribute(
                        "aria-selected",
                        !isOpen && tab === categoryTab ? "true" : "false",
                    );
                });
            eventCard
                ?.querySelectorAll("[data-schema-category-panel]")
                .forEach((panel) => {
        ...
        const syncCategoryStatus = (panel) => {
            ...
            const tab = eventCard?.querySelector(
            ...
            if (toggleLabel) {
                toggleLabel.textContent = enabledInput?.checked
                    ? "Kategori Aktif"
                    : "Kategori Nonaktif";
            }
        };
        ...
        const syncFormPreview = (panel) => {
            ...
            const selectedFields = [
                ...panel.querySelectorAll(".schema-field-button input:checked"),
            ].map((input) => ({
                label: input.dataset.previewLabel,
                required: input.dataset.primaryField === "true",
            }));
            const customFields = [
                ...panel.querySelectorAll("[data-custom-preview-field]"),
            ]
        ...
        const shouldBindRegistrationBuilderEvents =
        ...
        if (shouldBindRegistrationBuilderEvents) {
            ...
            document.addEventListener("click", (event) => {
                const categoryTab = event.target.closest(
                    "[data-schema-category-tab]",
                );
                const addCategoryButton = event.target.closest(
                    "[data-add-category]",
                );
                const removeCategoryButton = event.target.closest(
                ...
                if (addCategoryButton) {
                    const eventCard =
                        addCategoryButton.closest(".event-schema-card");
                    const nextCategoryTab = eventCard?.querySelector(
                        "[data-additional-category][hidden]",
                    );
                    ...
                    const panel = eventCard.querySelector(
                        `#${nextCategoryTab.getAttribute("aria-controls")}`,
                    );
                    const enabledInput = panel?.querySelector(
                        'input[name$="[enabled]"]',
                    );
                    ...
                    addCategoryButton.hidden = !eventCard.querySelector(
                        "[data-additional-category][hidden]",
                    );
                }
                ...
                if (removeCategoryButton) {
                    const eventCard =
                        removeCategoryButton.closest(".event-schema-card");
                    const panelId =
                        removeCategoryButton.getAttribute("aria-controls");
                    ...
                    const addControl = eventCard?.querySelector(
                        "[data-add-category]",
                    );
                    ...
                    if (categoryTabToRemove) {
                        ...
                        categoryTabToRemove.setAttribute(
                            "aria-selected",
                            "false",
                        );
                    }
            ...
            document.addEventListener("change", (event) => {
                const enabledInput = event.target.closest(
                    '[data-schema-category-panel] input[name$="[enabled]"]',
                );
                const categoryName = event.target.closest(
                    "[data-schema-category-panel] .schema-category-name input",
                );
                const registrationField = event.target.closest(
                    "[data-schema-category-panel] .schema-field-button input",
                );

                if (enabledInput) {
                    syncCategoryStatus(
                        enabledInput.closest("[data-schema-category-panel]"),
                    );
                }

                if (categoryName) {
                    const panel = categoryName.closest(
                        "[data-schema-category-panel]",
                    );
                    const tabLabel = panel
                        .closest(".event-schema-card")
                        ?.querySelector(
                            `[aria-controls="${panel.id}"] [data-schema-category-label]`,
                        );

                    if (tabLabel) {
                        tabLabel.textContent =
                            categoryName.value.trim() || "Kategori tanpa nama";
                    }

                    const panelTitle = panel.querySelector(
                        "[data-schema-panel-title]",
                    );
                    if (panelTitle) {
            ...
            document.addEventListener("input", (event) => {
                const customField = event.target.closest(
                    "[data-schema-category-panel] [data-custom-preview-field]",
                );

                if (customField) {
                    syncFormPreview(
                        customField.closest("[data-schema-category-panel]"),
                    );
                }
            });
        }

        const websiteSettingsForm = document.querySelector(
            "[data-website-settings-form]",
        );
        const formChangeStatus = websiteSettingsForm?.querySelector(
        ...
        if (
            ...
            websiteSettingsForm.dataset.settingsInitialized = "true";
            const tabs = [
                ...websiteSettingsForm.querySelectorAll("[data-cms-tab]"),
            ];
            const panels = [
                ...websiteSettingsForm.querySelectorAll("[data-cms-panel]"),
            ];
            const tabSelect = websiteSettingsForm.querySelector(
                "[data-cms-tab-select]",
            );
            const submitButton = websiteSettingsForm.querySelector(
            ...
            const markAsSaved = () => {
                isDirty = false;
                formChangeStatus.textContent =
                    "Semua perubahan yang tersimpan akan diterapkan ke website.";
            ...
            const syncPreview = (source) => {
                ...
                if (!key) return;
                websiteSettingsForm
                    .querySelectorAll(`[data-preview-target="${key}"]`)
                    .forEach((target) => {
            ...
            const syncCounter = (field) => {
                const counter = field.parentElement?.querySelector(
            ...
            const activateTab = (tabName, confirmChange = true) => {
                ...
                tabs.forEach((tab) =>
                    tab.setAttribute(
                        "aria-selected",
                        tab.dataset.cmsTab === tabName ? "true" : "false",
                    ),
                );
                panels.forEach((panel) => {
                    panel.hidden = panel.dataset.cmsPanel !== tabName;
                });
            ...
            tabs.forEach((tab) =>
                tab.addEventListener("click", () =>
                    activateTab(tab.dataset.cmsTab),
                ),
            );
            ...
            websiteSettingsForm
                .querySelectorAll("[data-cms-open-tab]")
                .forEach((button) => {
                    button.addEventListener("click", () =>
                        activateTab(button.dataset.cmsOpenTab),
                    );
                });
            ...
            websiteSettingsForm.addEventListener("change", (event) => {
                if (!event.target.matches("[data-cms-tab-select]"))
                    markAsChanged();
                syncPreview(event.target);
                syncCounter(event.target);
            });
            websiteSettingsForm.addEventListener("click", (event) => {
                if (
                    event.target.closest(
                        "[data-add-category], [data-remove-category], [data-add-custom-field], [data-remove-custom-field]",
                    )
                ) {
                    markAsChanged();
                }
            ...
            websiteSettingsForm.addEventListener(
                ...
                (event) => {
                    const invalidPanel =
                        event.target.closest("[data-cms-panel]");
    ...
    const setupTicketScanner = () => {
        ...
        const resultTitle = document.querySelector(
            "[data-ticket-result-title]",
        );
        const resultMessage = document.querySelector(
            "[data-ticket-result-message]",
        );
        const resultDetails = document.querySelector(
            "[data-ticket-result-details]",
        );
        ...
        const startScanner = async () => {
            ...
            try {
                ...
                reader.innerHTML =
                    '<p class="ticket-scanner-fallback">Kamera tidak tersedia. Gunakan input kode tiket manual.</p>';
            }
        };
        ...
        const showResult = (success, payload) => {
            ...
            if (success) {
                resultDetails.querySelector(
                    "[data-ticket-participant]",
                ).textContent = payload.participant_name;
                resultDetails.querySelector("[data-ticket-event]").textContent =
                    payload.event_name;
                resultDetails.querySelector("[data-ticket-code]").textContent =
                    payload.ticket_code;
                resultDetails.querySelector("[data-ticket-time]").textContent =
        ...
        const processCheckIn = async () => {
            ...
            try {
                ...
                showResult(false, {
                    message:
                        "Tiket belum dapat diverifikasi. Periksa koneksi lalu coba lagi.",
                });
            } finally {
                loading.hidden = true;
            }
    ...
    const setupTicketDetails = () => {
        document
            .querySelectorAll("[data-expandable-description]")
            .forEach((description) => {
                const toggle = description.querySelector(
                    "[data-description-toggle]",
                );
                const content = description.querySelector("p");

                if (!toggle || !content || description.dataset.expandBound)
                    return;
        ...
        document
            .querySelectorAll("[data-participant-accordion]")
            .forEach((accordion) => {
                ...
                if (!toggle || !panel || accordion.dataset.accordionBound)
                    return;
    ...
    const setupTicketModals = () => {
        document.querySelectorAll("[data-ticket-modal]").forEach((modal) => {
            ...
            const openButton = document.querySelector(
                `[data-ticket-modal-open="${modal.id}"]`,
            );
            const closeButtons = modal.querySelectorAll(
                "[data-ticket-modal-close]",
            );
        ...
        document
            ...
            .forEach((form) => {
                ...
                const modal = document.querySelector(
                    "[data-registration-confirm-modal]",
                );
                const summary = modal?.querySelector(
                    "[data-registration-confirm-summary]",
                );
                const confirmButton = modal?.querySelector(
                    "[data-registration-confirm-submit]",
                );
                const closeButtons = modal?.querySelectorAll(
                    "[data-registration-confirm-close]",
                );
                ...
                const openModal = () => {
                    ...
                    form.querySelectorAll(
                        "[data-confirm-label]:not([disabled])",
                    ).forEach((field) => {
    ...
    const initialize = () => {
        ...
        document
            .querySelectorAll("form[data-disable-submit]")
            .forEach((form) => {
...
document.addEventListener("DOMContentLoaded", () => {
    const readMoreButtons = document.querySelectorAll(".upcoming-read-more");

    readMoreButtons.forEach((button) => {
        button.addEventListener("click", () => {
            const currentContent = button.closest(".upcoming-simple-content");
            const currentDescription =
                currentContent?.querySelector(".event-description");
            ...
            const willExpand =
                !currentDescription.classList.contains("is-expanded");

            // Tutup semua card Event Mendatang terlebih dahulu
            readMoreButtons.forEach((otherButton) => {
                const otherContent = otherButton.closest(
                    ".upcoming-simple-content",
                );
                const otherDescription =
                    otherContent?.querySelector(".event-description");
                const otherLabel = otherButton.querySelector(
                    ".upcoming-read-more-label",
                );
                const otherIcon = otherButton.querySelector(
                    ".upcoming-read-more-icon",
                );

                if (!otherDescription || !otherLabel || !otherIcon) {
                    return;
                }

                otherDescription.classList.remove("is-expanded");
                otherButton.setAttribute("aria-expanded", "false");
                otherLabel.textContent = "Baca selengkapnya";
                otherIcon.classList.remove("is-rotated");
            });

            // Buka card yang dipilih jika sebelumnya tertutup
            if (willExpand) {
                const label = button.querySelector(".upcoming-read-more-label");
                const icon = button.querySelector(".upcoming-read-more-icon");

                currentDescription.classList.add("is-expanded");
