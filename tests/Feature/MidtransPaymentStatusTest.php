...
class MidtransPaymentStatusTest extends TestCase
{
    ...
    public function test_owner_can_refresh_a_midtrans_payment_status(): void
    {
        ...
        $this->actingAs($owner)
            ...
            ->assertSee('Periksa Detail Transaksi')
            ->assertSee('Tanggal')
            ->assertSee($event->starts_at->translatedFormat('d F Y'))
            ->assertSee('Waktu')
            ->assertSee($event->starts_at->format('H.i').' WIB')
            ->assertSee('Lokasi')
            ->assertSee($event->location)
            ->assertSee('Rp75.000')
            ->assertSee('Memverifikasi pembayaran...')
            ->assertSee('Status Belum Dapat Diperiksa')
            ->assertSee('Kami belum dapat memverifikasi pembayaran saat ini. Silakan coba kembali beberapa saat lagi.')
            ->assertDontSee('Cek Status Pembayaran')
            ->assertDontSee('Setelah pembayaran, status akan diperiksa otomatis.')
            ->assertDontSee('Respons status Midtrans tidak lolos validasi keamanan.')
