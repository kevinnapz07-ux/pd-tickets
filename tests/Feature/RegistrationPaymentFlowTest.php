...
class RegistrationPaymentFlowTest extends TestCase
{
    ...
    public function test_ticket_list_uses_real_check_in_status_and_detail_has_no_self_link(): void
    {
        ...
        $this->actingAs($owner)
            ...
            ->assertSee('registration-summary', false)
            ->assertSee('<dt>Event</dt>', false)
            ->assertSee('<dt>Nama</dt>', false)
            ->assertSee('<dt>Status</dt>', false)
            ->assertDontSee($waiting->name.' terdaftar untuk')
            ->assertSee('Informasi Registrasi')
